...
class Generator(nn.Module):
    def __init__(self, ch=1, nz=100, nf=64):
        super().__init__()
        self.main = nn.Sequential(
            ...
            nn.ConvTranspose2d(nf, ch, kernel_size=1, stride=1, padding=2, bias=False),
            nn.Tanh()
        )
...
class Discriminator(nn.Module):
    def __init__(self, ch=1, nf=64):
        ...
        self.main = nn.Sequential(
            ...
            nn.Conv2d(nf*4, 1, kernel_size=4, stride=2, padding=1, bias=False),
            nn.Sigmoid()
        )
...
model = DCGAN(**params)
model.scheduler_g = torch.optim.lr_scheduler.StepLR(model.optim_g, step_size=20, gamma=0.5)
model.scheduler_d = torch.optim.lr_scheduler.StepLR(model.optim_d, step_size=20, gamma=0.5)